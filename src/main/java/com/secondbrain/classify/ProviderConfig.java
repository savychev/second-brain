...
public final class ProviderConfig {
    ...
    /**
     * Предел ожидания ответа от <b>сетевого</b> провайдера. Выведен из цели PRD
     * «захват за ≤10 секунд»: не ответил вовремя — классифицируем правилами
     * и не задерживаем человека.
     */
    private static final Duration DEFAULT_TIMEOUT = Duration.ofSeconds(8);

    /**
     * Предел ожидания для локальной модели — заметно больше сетевого.
     *
     * <p>Восемь секунд оказались ошибкой применения правильного числа не к тому
     * случаю. Цель PRD «захват за 10 секунд» — про усилие человека: написал
     * и забыл. Отправив мысль в Telegram, он свою часть уже закончил, и ответ,
     * пришедший на десятой секунде, не стоит ему ничего. А вот классификация
     * правилами вместо модели стоит — каждый день, молча.
     *
     * <p>Замерено: запрос к загруженной модели ~2,4 с, к выгруженной ~11 с.
     * Выгружается она неизбежно — Ollama освобождает память, компьютер уходит
     * в сон, — и первая мысль дня попадает именно в этот случай. Тридцать
     * секунд покрывают загрузку с запасом на холодный диск после пробуждения.
     *
     * <p>Подстраховка правилами никуда не делась: она теперь срабатывает,
     * когда модель действительно не работает, а не когда она просто просыпается.
     */
    private static final Duration DEFAULT_OLLAMA_TIMEOUT = Duration.ofSeconds(30);

    /**
     * Сколько Ollama держит модель в памяти после запроса.
     *
     * <p>Своё значение Ollama держит 5 минут. Замерено: запрос к загруженной
     * модели ~2,4 секунды, к выгруженной ~11. Разница в скорости, а не в
     * работоспособности — за корректность отвечает {@link #DEFAULT_OLLAMA_TIMEOUT},
     * которого хватает и на холодный запуск.
     *
     ...
     *
     * <p>Круглосуточно удержать её всё равно не выйдет: компьютер уходит в сон,
     * система вытесняет память. Поэтому холодный запуск — не исключение,
     * которое надо предотвратить, а обычный случай, который надо пережить.
     */
    ...
    public static ProviderConfig load(Path configFile) {
        ...
        return switch (provider) {
            // Локальной модели даём больше времени, чем сетевой: она может
            // подгружаться с диска, а сетевая либо отвечает, либо нет.
            case OLLAMA -> new ProviderConfig(OLLAMA, null,
                    orDefault(value("OLLAMA_MODEL", props, "ollama.model"), DEFAULT_OLLAMA_MODEL),
                    (timeout == null) ? DEFAULT_OLLAMA_TIMEOUT : timeout,
