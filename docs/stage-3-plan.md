...
> ⚠️ **Исправлено после ревью 31.07.2026.** Первоначальный замысел импортёра был неверен и
> при повторном запуске создавал бы дубликаты страниц в Notion. Дефект подтверждён
> воспроизведением, см. `SaveSemanticsTest`.
>
> Ошибка была в предположении, что `save()` донесёт `notionPageId` при повторном импорте.
> Не донесёт: `ON CONFLICT(id) DO NOTHING` отбрасывает входящую строку **целиком**. Сценарий
> дубля: заметка импортирована без page id → Notion починился → `--sync` проставил page id
> **в JSON** → повторный импорт `DO NOTHING` → в SQLite по-прежнему NULL → после переключения
> очередь публикует её второй раз.
>
> Исправлять надо импортёр, а не `save()`: `save()` документирован как «вставить, если новая»,
> и делает именно это. Размывать его смысл условным обновлением — значит получить метод,
> который для одних полей вставка, а для других обновление.
...
public static Report importAll(Path jsonFile, NoteRepository target) {
    ...
    for (Note n : source) {
        target.save(n);              // ВСЕ семь полей; НЕ через CaptureService и не Note.create()
        if (n.isSynced()) {
            // Обязательно: если заметка уже была в базе без page id,
            // save() его не донесёт. markSynced умеет ровно то, что нужно —
            // проставить, но никогда не перезаписать.
            target.markSynced(n.id(), n.notionPageId());
        }
    }
    return new Report(...);
}
...
Три слоя защиты: (1) `notionPageId` переносится и доливается через `markSynced` →
`findUnsynced()` эти заметки не вернёт; (2) `save()` идемпотентен, `markSynced` не
перезаписывает → импорт можно гонять сколько угодно раз; (3) `notes.json` не удаляется
и не меняется — откат остаётся рабочим бессрочно.
...
> ⚠️ Осторожно с последней командой: `--sync` не проверяет очередь, а **разгребает** её.
> Если `notionPageId` где-то потерялся, она не предупредит — она создаст дубликат.
> Поэтому перед ней обязательна проверка через `sqlite3` (строка выше) или `:stats`,
> где видно «Ждёт отправки в Notion: 0».

Приёмка миграции — это счётчик очереди, равный нулю, ДО всякой досылки. Потеряйся хоть один
`notionPageId` — это видно сразу, на двух заметках, с целым `notes.json` под рукой.
